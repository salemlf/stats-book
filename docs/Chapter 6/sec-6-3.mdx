...
<details>
  <summary>Solution</summary>
  <div>
  Note that $x < 0 < \beta \Longleftrightarrow \frac{1}{x} < 0 < \beta$, that $0 \leq x \leq \beta^{-1} \Longrightarrow 0 \leq \beta \leq \frac{1}{x}$, and $x > \beta^{-1} \Longleftrightarrow \beta > \frac{1}{x}$.
  Thus,
  $$
  \begin{align*}
    F_{X}(x) & = P(X \leq x)\\
...
[<em>Hint</em>: Recall Exercise 4.5.]
